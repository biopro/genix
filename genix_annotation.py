...
RNAMMER_PATH = '/home/frederico/Programas/RNAmmer/./rnammer'
...
for sequencia in sequencias:
	...
	for prodigal_gene_index, prodigal_gene in enumerate(prodigal_genes):
		...
		if blast_hits_query[0][0] > 1 and not corrected:
		...
		if blast_hits_query[0][1] > 1 and not corrected:
			closest_gene = prodigal_gene
			if prodigal_gene[4] == '+':
				database_cursor.execute('SELECT end FROM raw_features WHERE sequence_id =? AND start < ? ORDER BY start',(prodigal_gene[1],prodigal_gene[2],))
				...
				database_cursor.execute('SELECT start FROM raw_features WHERE sequence_id =? AND start > ? ORDER BY start',(prodigal_gene[1],prodigal_gene[2],))
				query_result = database_cursor.fetchall()
				if query_result:
